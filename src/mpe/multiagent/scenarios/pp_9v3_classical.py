...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        self.prey_size = 0.05
        # [vel+pos+relative pos to all landmarks+relative pos to agents -i + velocity of preys], 2+2+2X2+2X11+2x3=36
        self.local_state_size = 36
        ...
        for other in world.agents:
            ...
            if agent.view_radius >= 0 and dist <= agent.view_radius:
                ...
                if not other.adversary:
                    other_vel.append(other.state.p_vel)                     # 2*2
            else:
                other_pos.append(np.array([0., 0.]))
                if not other.adversary:
                    other_vel.append(np.array([0., 0.]))
        # 2+2+2*(num_landmarks:2)+2*(num_agents-1)+2*3=4+4+22+6=36
