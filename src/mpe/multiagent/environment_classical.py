...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, scenario=None, reset_callback=None, reward_callback=None,
        ...
        print("Use classical environments.")
        self.world = world
        ...
        self.num_landmarks = scenario.num_landmarks
        self.episode_limit = scenario.episode_limit
        self.current_step = 0
        # scenario callbacks
        ...
        # 定义states, obs_n为全局变量，方便episode_runner调用
    ...
    def step(self, action_n):
        ...
        # advance world state
        self.world.step()
        # global states为 num_agents X local_state_size 维度
        for agent in self.agents:
            obs_n.append(self._get_obs(agent))
            reward_n.append(self._get_reward(agent))
            done_n.append(self._get_done(agent))
        ...
        self.current_step += 1
        # return global_states, obs_n, reward_n, done_n, info_n
    ...
    def get_avail_actions(self):
        ...
        avail_actions = []
        for agent_id in range(self.scenario.num_agents):
            if self.discrete_action_space:
                avail_act = [1, 1, 1, 1, 1]
            else:
    ...
    def get_env_info(self):
        if self.discrete_action_space:
            env_info = {"state_shape": self.scenario.state_size,
                        "obs_shape": self.scenario.local_state_size,
                        "n_actions":  self.world.dim_p * 2 + 1,
                        "n_agents": self.scenario.num_agents,
                        "n_entities": self.scenario.num_agents + self.scenario.num_landmarks,
